...
public class TestHttpAuthorityServer {
    ...
    public TestHttpAuthorityServer(final UserStore<TestUser, TestUserInfo, TestUserCredentials> userStore, final SessionStore<TestSession> sessionStore, final SessionCreationPolicy sessionAccess, final ClientIdPolicy clientIdPolicy) throws IOException {
        _server = HttpServer.create(new InetSocketAddress(8000), 0);
        _objectMapper = new ObjectMapper();
        _authority = new Authority<>(userStore, sessionStore, sessionAccess, clientIdPolicy, TestKeyPairProvider.create());
